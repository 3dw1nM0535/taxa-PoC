...
contract("Farm test", async accounts => {
  ...
  it("should register a farm", async() => {
    const result = await instance.registerFarm(100, "John Doe", "farm_image_hash", "36.89384", "-1.29473", 884565);
    assert.equal(Number(result.logs[1].args._farmSize), 100, "farm size should be 100");
    assert.equal(String(result.logs[1].args._farmOwner), "John Doe", "farm owner should be John Doe");
  ...
  it("should not create farm with no farm size", async() => {
    try {
      await instance.registerFarm(0, "John Doe", "farm_image_hash", "36.89384", "-1.29473", 884565);
    } catch(err) {
  ...
  it("should not create farm without image hash", async() => {
    try {
      await instance.registerFarm(100, "John Doe", "", "36.89384", "-1.29473", 884565);
    } catch(err) {
  ...
  it("farm longitude coordinate should be known", async() => {
    try {
      await instance.registerFarm(100, "John Doe", "farm_image_hash", "", "-1.29473", 884565);
    } catch(err) {
  ...
  it("farm latitude coordinate should be known", async() => {
    try {
      await instance.registerFarm(100, "John Doe", "farm_image_hash", "36.89384", "", 884565);
    } catch(err) {
